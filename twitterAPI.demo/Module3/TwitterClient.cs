...
namespace twitterAPI.demo.Module3
{
    public class TwitterClient
    {
        ...
        const string _SearchTweets = "https://api.twitter.com/1.1/search/tweets.json";
        ...
        public string UserLookup(string address)
        {
            WebRequest request = WebRequest.Create(address);
        ...
        public TweetSearchResponseDTO SearchForTweets(string query,
                                                      int numberofTweets,
                                                      string language)
        {
            string responseJSON = SendGET(_SearchTweets
                                    + "?q=" + query
                                    + "&count=" + numberofTweets
                                    + "&lang=" + language);

            TweetSearchResponseDTO responseDTO =
                JsonConvert.DeserializeObject<TweetSearchResponseDTO>(responseJSON);
